...
// The TransientDB struct is a wrapper around the DataStore struct. It is used by
// the executor to  store the temporarily state generated by the transactions.
// It will be discarded after the transactions are executed. The state changes will be committed
// to the readonlyParent by the state committer. TransientDB never writes back to the readonlyParent
...
// NewTransientDB creates a new TransientDB instance.
...
func (this *TransientDB) Preload(key []byte) interface{} { return this.readonlyParent.Preload(key) }
