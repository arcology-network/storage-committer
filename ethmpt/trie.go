...
func NewTrie() *Trie {
	trie := &Trie{NewBranchNode()}
	// for i := 0; i < 16; i++ {
	// 	trie.Put([]byte{uint8(i)}, []byte{uint8(i)})
	// }
	return trie
}

func (t *Trie) Root() Node            { return t.root }
func (t *Trie) SetRoot(rootNode Node) { t.root = rootNode }

// func (this *Trie) Init(depth int) []byte {
// 	for i := 0; i < 16; i++ {
// 		(this.root.(*BranchNode)).Branches[i] =
// 	}
// 	return t.root.Hash()
// }
