...
func (this *Bytes) StorageEncode(key string) []byte {
	...
	// back to a 32 byte array. However, we don't have a fixed word size for entries generated by Arcology's extensions,
	// so we need to encode the length of the byte array.
	if committercommon.GetPathType(key) == committercommon.ETH_PATH_TYPE {
