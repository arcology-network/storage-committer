...
func TestSimpleBalance(t *testing.T) {
	...
	url2.Write(1, "blcc://eth1.0/account/"+alice+"/balance", commutative.NewBalance(big.NewInt(200), big.NewInt(10)))
	balance, _ = url2.Read(1, "blcc://eth1.0/account/"+alice+"/balance")
	if balance.(*commutative.Balance).Value().(*big.Int).Cmp(big.NewInt(43)) != 0 {
		t.Error("Error: Wrong blcc://eth1.0/account/alice/balance value")
	}
