...
type StateCommitter struct {
	...
	indexer     *importer.AccountIndexer // Account indexer is an index by unique account address.
...
func NewStorageCommitter(store interfaces.Datastore) *StateCommitter {
	...
	inserter := func(seq *importer.DeltaSequence, update *importer.AccountUpdate) *importer.AccountUpdate {
		if update == nil {
			key := platform.GetAccountAddr(seq.Key)
			addr, _ := hexutil.Decode(platform.GetAccountAddr(key))
			store.(*storage.EthDataStore).NewAccount(addr)

			return &importer.AccountUpdate{
				Key:  key,
	...
	return &StateCommitter{
		...
		indexer:     importer.NewAccountIndexer(store, platform, keyGetter, inserter),
	}
}
