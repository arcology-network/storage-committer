...
func (this *Path) Set(value any, source any) (any, uint32, uint32, uint32, error) {
	...
	writeCache := source.([]any)[3].(interface {
		Write(tx uint64, key string, value any) (int64, error)
		InCache(path string) (any, bool)
	})
...
// AffliatedDeletes returns the keys to be deleted when the path is deleted.
func (this *Path) AffliatedDeletes() []string {
	return this.DeltaSet.Committed().Elements()
}
...
func (this *Path) CommittedKeys() []string { // Committed keys
